...
| # | Item | Status | Notes |
|---|---|---|---|
| 5 | On-demand model download instead of bundling all languages in the APK | ✅ Done | Only Persian ships in the APK now (`VoiceModel.bundledInApk`); English/Swedish are fetched by the new `ModelDownloader` — same `k2-fsa/sherpa-onnx` release `.tar.bz2` bundles `download_assets.sh` used to pre-fetch, now extracted on-device with `commons-compress` (tar+bzip2) the first time that language is actually requested, atomically renamed into place, cached forever after. Debug APK went from ~199MB of bundled TTS assets to 89MB total. Needs `INTERNET` permission (added). Verified live end-to-end: fresh install → tap English → `ModelDownloader` logs the fetch → first attempt times out at the existing 10s `awaitEngine` wait and surfaces "downloading, try again" via `MainActivity`'s new `onError` listener → download completes in the background (~60MB in ~35s on this network) → second tap synthesizes instantly → app restart confirms the cached model isn't re-downloaded. |
